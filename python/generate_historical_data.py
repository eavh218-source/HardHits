...
def generate_predictions_for_date(target_date):
    ...
    for game in games[:8]:  # Limit to first 8 games
        ...
        for side in ['away', 'home']:
            ...
            for player in roster[:9]:  # Top 9 hitters
...
def fetch_results_for_date(target_date):
    ...
    target_date_str = target_date.strftime('%Y-%m-%d')
    print(f"  Fetching results for {target_date_str}...")
    
    try:
        data = pb.statcast(start_dt=target_date_str, end_dt=target_date_str)
        hrs = data[data['events'] == 'home_run'].copy()
    except Exception as e:
        print(f"  Warning: Could not fetch results for {target_date_str}: {e}")
        return []
...
def run_historical_generation():
    ...
    while current_date < END_DATE:
        ...
        print(f"  ✓ Fetched {len(results)} actual home runs")
    ...
    print(f"\n✅ Saved {len(all_predictions)} historical predictions to hr_model_historical.js")
    ...
    print(f"✅ Saved {len(all_results)} historical results to hr_results_historical.js")
